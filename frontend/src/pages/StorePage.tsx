...
import { getProducts, createProduct, updateProduct, deactivateProduct, reactivateProduct, getCategories, type Product } from "../services/product.service";
...
export default function StorePage() {
    ...
    // Product CRUD
    const openNewProd = () => { setEditingProd(null); setProdForm(emptyProduct); setProdDrawer(true); };
    const openEditProd = (p: Product) => { setEditingProd(p.id); setProdForm({ name: p.name, description: p.description ?? "", price: p.price, stock: p.stock, category: p.category }); setProdDrawer(true); };
                ...
                {tab === "products" ? (
                    <>
                        {loading ? (
                            ...
                            <p style={styles.empty}>{search || catFilter ? "No hay productos con esos filtros." : "No hay productos registrados."}</p>
                        ) : (
                                    ...
                                    <tbody>{paginated.map((p) => {
                                        ...
                                        return (
                                            <tr key={p.id} style={styles.row} className="store-row">
                                                <td style={{ ...styles.td, paddingLeft: 16, fontWeight: 500 }}>{p.name}</td>
                                                <td style={{ ...styles.td, ...styles.muted }}>{p.category}</td>
                                                <td style={styles.td}>${p.price}</td>
                                                <td style={styles.td}>
                                                    <span style={{ color: lowStock ? "#c0392b" : inactive ? "#ccc" : "#3a7d44", fontWeight: 500 }}>
                                                        {p.stock}
                                                    </span>
                                                </td>
                                                <td style={styles.td}>
                                                ...
                                                <td style={styles.td}>
                                                    <div className="actions-group" style={{ display: "flex", gap: 4 }}>
                                                        {inactive ? (
                                                            ...
                                                            <>
                                                                <button className="btn-icon-action" title="Editar" style={styles.btnIconAction}
